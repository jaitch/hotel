  ...
  class FrontDesk
    ...
    def make_reservation(date_range_object)
      @all_rooms.each do |room|
        if room.is_available_for_date_range?(date_range_object)
          reservation = Hotel::Reservation.new(date_range_object)
          room.occupied_date_ranges << date_range_object
    ...
    def make_room_block(num_rooms, date_range_object)
      ...
      available_rooms_array = @all_rooms.select { |room| room.is_available_for_date_range?(date_range_object)
      }
      if num_rooms <= available_rooms_array.length
        available_rooms_array.each do |room|
          room.blocks << date_range_object
        end
    ...
    def available_rooms_given_date(date_sought)
      @all_rooms.select { |room| room.is_available_on_date?(date_sought)
      }.map { |room| room.number
      }
      # Still need to make blocked dates unavailable
    end
